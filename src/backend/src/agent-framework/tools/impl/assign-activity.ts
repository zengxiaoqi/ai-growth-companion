...
export class AssignActivityTool extends BaseTool<AssignActivityInput> {
  ...
  readonly metadata: ToolMetadata = {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        parentId: { type: 'number', description: '家长ID（通常由运行时自动注入）' },
        ...
        confirmPublish: { type: 'boolean', description: 'false=生成草稿，true=确认发布' },
  ...
  private buildDraft(input: Omit<PendingAssignmentDraft, 'createdAt' | 'expiresAt'>): PendingAssignmentDraft {
    const now = Date.now();
    return {
      ...input,
  ...
  private resolvePublishPayload(
    ...
    if (!args.childId || !args.activityType || !args.topic || !Number.isFinite(Number(args.difficulty))) {
      return null;
    }
    const ageGroup = this.resolveAgeGroup(args.ageGroup, Number(args.difficulty));
