...
export class LearningController {
  ...
  private resolveChildId(req: any, childId?: string): number {
    const viewerType = req.user?.type;
    const numeric = viewerType === 'child' ? Number(req.user?.sub) : Number(childId);
  ...
  async complete(@Request() req: any, @Param('id') id: string, @Body() body: CompleteLearningDto) {
    ...
    const durationSeconds =
      requestedDuration && Number.isFinite(requestedDuration)
    ...
    if (updatedRecord) {
      try {
        const content = await this.contentRepo.findOne({
          where: { id: updatedRecord.contentId },
        });
  ...
  async getDraftLessons(@Request() req: any, @Query('childId') childId: string) {
  ...
  async updateDraft(@Request() req: any, @Param('id') id: string, @Body() body: UpdateDraftDto) {
    if (req.user?.type !== 'parent') {
  ...
  async modifyLesson(@Request() req: any, @Param('id') id: string, @Body() body: ModifyDraftDto) {
    const parentId = req.user?.sub;
    if (req.user?.type !== 'parent') {
  ...
  async approveVideo(@Request() req: any, @Param('id') id: string, @Body() body: ApproveVideoDto) {
    if (req.user?.type !== 'parent') {
