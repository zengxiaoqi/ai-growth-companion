...
Stream<Map<String, dynamic>> platformFetchSseStream({
  ...
  try {
    ...
    yield {'type': 'error', 'message': '网络请求失败: $e'};
    return;
  }
  ...
  if (readableStream == null) {
    ...
    yield {'type': 'error', 'message': '响应无流数据'};
    return;
  }
  ...
  try {
    while (true) {
      ...
      // result.value 是 JSAny?，需要转成 Uint8List
    ...
    _log.warning('Web SSE: read error: $e');
    yield {'type': 'error', 'message': '流读取错误: $e'};
  } finally {
    try {
      reader.releaseLock();
    } catch (_) {}
