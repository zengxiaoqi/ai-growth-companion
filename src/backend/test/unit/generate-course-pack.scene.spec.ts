...
describe('GenerateCoursePackTool scene output', () => {
  ...
  it('adds native watch/write/practice scene documents even when llm omits them', async () => {
    generateActivityTool.execute.mockResolvedValueOnce(JSON.stringify({
      ...
      title: 'Season Quiz',
      questions: [
        { question: 'Which season has snow?', options: ['spring', 'summer', 'winter'], correctIndex: 2 },
      ],
    }));
    ...
    llmClient.generate.mockResolvedValueOnce(JSON.stringify({
      ...
      modules: {
        listening: {
          goal: 'listen',
          audioScript: [{ segment: 'intro', narration: 'Let us watch seasons', durationSec: 8 }],
    ...
    expect(result.write?.scene?.scenes?.[0]?.interaction?.type).toBe('trace_path');

    expect(result.practice?.scene?.stepType).toBe('practice');
    expect(result.practice?.scene?.mode).toBe('activity_shell');
    expect(result.practice?.scene?.scenes?.[1]?.interaction?.type).toBe('launch_activity');
    expect(result.practice?.scene?.scenes?.[1]?.fallbackActivity?.activityType).toBe(result.game?.activityType);
    expect(result.practice?.scene?.scenes?.[1]?.fallbackActivity?.activityData).toEqual(result.game?.activityData);
  });
  ...
  it('backfills watch scene animation templates when native watch scenes omit them', async () => {
    generateActivityTool.execute.mockResolvedValueOnce(JSON.stringify({
      type: 'quiz',
      title: 'Word Quiz',
      questions: [{ question: '念一念这个字', options: ['河', '海'], correctIndex: 0 }],
    }));
    ...
    llmClient.generate.mockResolvedValueOnce(JSON.stringify({
      ...
      modules: {
        listening: { goal: 'listen', audioScript: [{ segment: 'intro', narration: '河字', durationSec: 8 }], questions: [] },
        speaking: { goal: 'speak', warmup: '说一说', prompts: [{ prompt: '这个字是什么', sampleAnswer: '河' }] },
        reading: { goal: 'read', text: '河', keywords: ['河'], questions: [] },
        writing: { goal: 'trace', tracingItems: ['河'], practiceTasks: ['描一描'], checklist: ['跟着描'] },
      },
      watch: {
        scene: {
          version: 1,
    ...
    const riverScene = result.watch?.scene?.scenes?.find((scene: any) => {
      ...
      return source.includes('河字') || source.includes('“河”') || source.includes('"河"');
    });

    expect(['language.character-stroke', 'language.word-reveal']).toContain(riverScene?.visual?.templateId);
