...
describe('AiService course pack export', () => {
  ...
  it('exports bilingual subtitle srt and falls back to same text when EN is missing', async () => {
    learningArchiveService.getStudyPlanById.mockResolvedValue(mockCoursePackRow());
    ...
    expect(String(exported.body)).toContain('Sun Moon Mountains Rivers\nSun Moon Mountains Rivers');
  ...
  it('includes bilingual subtitle file inside bundle zip', async () => {
    ...
    const bilingualName = names.find((n) => n.endsWith('-subtitle-bilingual.srt'));
  ...
  it('exports multiple records into one batch zip with manifest', async () => {
    learningArchiveService.getStudyPlanById.mockImplementation(async (id: number) => {
    ...
    const zip = await JSZip.loadAsync(exported.body as Buffer);
    const names = Object.keys(zip.files);
    expect(names.some((n) => n.includes('Chinese-Lesson-11/Chinese-Lesson-capcut.json'))).toBe(true);
    expect(names.some((n) => n.includes('Chinese-Lesson-11/Chinese-Lesson-subtitle-bilingual.srt'))).toBe(true);
    expect(names.some((n) => n.includes('Math-Video-12/Math-Video-capcut.json'))).toBe(true);
    expect(names.some((n) => n.includes('Math-Video-12/Math-Video-subtitle-bilingual.srt'))).toBe(true);
  ...
  it('generates weekly plan with multiple saved course packs', async () => {
    ...
    let recordSeed = 100;
    learningArchiveService.createStudyPlanRecord.mockImplementation(async () => ({
      id: ++recordSeed,
    }));
    ...
    expect(learningArchiveService.createStudyPlanRecord).toHaveBeenCalledTimes(3);
  });
  ...
  it('builds TTS narration without cue labels and promotes subtitle text over generic shot names', async () => {
    ...
    const segments = (serviceWithVoice as any).collectStoryboardVideoSegments(pack);
