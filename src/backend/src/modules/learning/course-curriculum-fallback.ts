...
function extractExampleFacts(blocks: Record<string, any>[]): Record<string, string> {
  ...
  return Object.fromEntries(
    pairs.filter((entry) => entry.unit && entry.fact).map((entry) => [entry.unit, entry.fact]),
  );
}
      ...
      return questions
        .map((item: any) => {
          ...
          const options = Array.isArray(item?.options)
            ...
            : [];
          const answer =
            typeof item?.answer === 'number' ? options[item.answer] || '' : toText(item?.answer);
...
function extractTopicTerms(source: string): string[] {
  ...
  const knownTerms = Object.keys(TOPIC_ALIASES).filter((term) => text.includes(term));
