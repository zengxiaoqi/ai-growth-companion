...
export class GenerateCoursePackTool {
  ...
  async execute(args: GenerateCoursePackArgs): Promise<string> {
    ...
    const gameBundle = normalized.includeGame ? await this.generateGameBundle(normalized) : null;
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        const coursePack = this.sanitizeCoursePack(normalized, parsed.value, gameBundle);
  ...
  ensureTeachingMediaPack(pack: Record<string, any>): Record<string, any> {
    ...
    next.visualStory =
      next.visualStory && typeof next.visualStory === 'object' ? next.visualStory : {};
    ...
    next.modules.listening =
      next.modules.listening && typeof next.modules.listening === 'object'
        ...
        : {};
    next.videoLesson =
      next.videoLesson && typeof next.videoLesson === 'object' ? next.videoLesson : {};
  ...
  private sanitizeCoursePack(
    ...
    const outcomes = this.toStringArray(
      ...
      curriculumSeed?.outcomes?.length
        ...
        : [`理解${args.topic}的重点内容`, '完成一次互动练习', '练习听、说、读、写'],
    );
    ...
    const pack: Record<string, any> = {
      ...
      modules: {
        ...
        reading: {
          ...
          text: this.toText(
            reading?.text,
            curriculumSeed?.readingText || `今天我们一起学习${args.topic}，请大声读一读。`,
          ),
  ...
  private shouldUseFallbackShots(shots: Array<Record<string, any>>, args: NormalizedArgs): boolean {
  ...
  private buildUnitVisualPrompt(unit: string, args: NormalizedArgs, _index: number): string {
    const literacyLike = this.isLiteracyLikeTopic(args.topic, args.focus);
    if (literacyLike && /^[\u4e00-\u9fff]{1,2}$/.test(unit)) {
  ...
  private buildUnitNarration(unit: string, args: NormalizedArgs, _index: number): string {
  ...
  private normalizeArgs(args: GenerateCoursePackArgs): NormalizedArgs {
    ...
    let safeFocus: CourseFocus = ['literacy', 'math', 'science', 'mixed'].includes(requestedFocus)
      ? requestedFocus
      : 'mixed';
    ...
    return {
      ...
      domain,
      gameType:
        this.toText((args as any)?.gameType) && ACTIVITY_TYPES.includes((args as any).gameType)
  ...
  private inferDomainFromText(source: string): CourseDomain {
    ...
    if (/科学|动物|植物|天气|季节|四季|自然|实验|观察|昼夜|太阳|月亮|地球|水循环/.test(text))
      return 'science';
  ...
  private toStringArray(value: any, minLen: number, fallback: string[]): string[] {
    const arr = Array.isArray(value) ? value.map((item) => this.toText(item)).filter(Boolean) : [];
