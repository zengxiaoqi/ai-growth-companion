...
function sanitizeVisual(raw: any): SceneVisual | undefined {
  ...
  const visual: SceneVisual = {
    ...
    templateParams:
      raw.templateParams && typeof raw.templateParams === 'object' ? raw.templateParams : undefined,
  };
  ...
  const timeline = value
    .map((entry: any) => {
      ...
      return {
        ...
        durationSec:
          entry?.durationSec != null ? toSafeInt(entry.durationSec, 1, 0, 60) : undefined,
...
function sanitizeInteraction(raw: any): SceneInteraction | undefined {
  ...
  if (type === 'trace_path') {
    ...
    return {
      ...
      targets,
      minCoverage:
        raw.minCoverage != null ? Math.max(0.1, Math.min(1, Number(raw.minCoverage))) : undefined,
    };
  }
...
function buildWatchTemplateHint(source: string, scene: AnyRecord, domain?: string): SceneVisual {
  ...
  const visual: SceneVisual = {
    ...
    items: toStringArray(source.match(/春|夏|秋|冬|花|太阳|树叶|雪花/g) || []).map(
      (label, index) => ({
        id: `item-${index + 1}`,
        label,
      }),
...
export function deriveWatchSceneDocument(
  ...
  const sourceScenes =
    Array.isArray(module?.visualStory?.scenes) && module.visualStory.scenes.length > 0
...
export function deriveWriteSceneDocument(writing: AnyRecord, topic: string): LessonSceneDocument {
