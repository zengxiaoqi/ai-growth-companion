...
describe("GenerateCoursePackTool", () => {
  ...
  let mockedGetCoursePackCurriculumSeed: jest.MockedFunction<
    typeof getCoursePackCurriculumSeed
  >;
  ...
  beforeEach(() => {
    ...
    mockedGetCoursePackCurriculumSeed =
      getCoursePackCurriculumSeed as jest.MockedFunction<
        typeof getCoursePackCurriculumSeed
      >;
      ...
      JSON.stringify({
        ...
        visualStory: {
          ...
          scenes: [
            {
              scene: "s1",
              imagePrompt: "rabbit with apples",
              narration: "开始",
              onScreenText: "开始",
              durationSec: 8,
            },
            ...
            {
              scene: "s3",
              imagePrompt: "interactive question",
              narration: "提问",
              onScreenText: "提问",
              durationSec: 10,
            },
  ...
  it("defaults ageGroup to 5-6 and falls back when llm keeps returning invalid JSON", async () => {
    ...
    expect(
      result.modules?.listening?.audioScript?.length,
    ).toBeGreaterThanOrEqual(3);
    expect(
      result.videoLesson?.shots?.some((shot: any) =>
        /opening|concept|practice|wrap-?up|shot\s*\d+/i.test(String(shot.shot)),
      ),
    ).toBe(false);
    expect(
      result.modules?.listening?.audioScript?.some((item: any) =>
        /part\s*\d+/i.test(String(item.segment)),
      ),
    ).toBe(false);
  ...
  it("expands explicit literacy units into per-item teaching shots", async () => {
    ...
    const shotNames =
      result.videoLesson?.shots?.map((shot: any) => String(shot.shot)) || [];
    const narrations =
      result.videoLesson?.shots?.map((shot: any) => String(shot.narration)) ||
      [];
    const audioSegments =
      result.modules?.listening?.audioScript?.map((item: any) =>
        String(item.segment),
      ) || [];
    ...
    expect(
      narrations.some(
        (text: string) => text.includes("“天”字") || text.includes("”天”字"),
      ),
    ).toBe(true);
    expect(
      narrations.some(
        (text: string) => text.includes("“地”字") || text.includes("”地”字"),
      ),
    ).toBe(true);
    expect(
      narrations.some(
        (text: string) => text.includes("“人”字") || text.includes("”人”字"),
      ),
    ).toBe(true);
  ...
  it("uses curriculum seed for fallback summary and review narration", async () => {
    ...
    const sceneNarrations =
      result.visualStory?.scenes?.map((s: any) => String(s.narration)) || [];
    const hasSpringNarration = sceneNarrations.some((t: string) =>
      t.includes("春"),
    );
    ...
    const shotNames =
      result.videoLesson?.shots?.map((s: any) => String(s.shot)) || [];
    ...
    const audioNarrations =
      result.modules?.listening?.audioScript?.map((a: any) =>
        String(a.narration),
      ) || [];
    expect(audioNarrations.some((t: string) => t.includes("春"))).toBe(true);

    // Review narration mentions units and facts
    const reviewScene = result.visualStory?.scenes?.find((s: any) =>
      String(s.scene).includes("总结"),
    );
    if (reviewScene) {
  ...
  it("uses curriculum seed for fallback reading text and writing items", async () => {
    ...
    expect(
      keywords.some((k: string) => k.includes("猫") || k.includes("狗")),
    ).toBe(true);
  });
});
