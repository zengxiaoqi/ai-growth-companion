...
export class AiController {
  ...
  @ApiOperation({
    summary:
      "导出课程包（CapCut JSON / 配音TXT / 配音MP3 / 教学视频MP4 / 分镜CSV / 字幕SRT / 双语SRT / 素材包ZIP）",
  })
  ...
  async enrichCoursePackBilingual(
    ...
    @Param("id") id: string,
    @Body()
    body: { saveAsVersion?: boolean; overwrite?: boolean; sessionId?: string },
  ) {
    try {
  ...
  @Post("course-pack")
  @ApiOperation({
    summary: "家长一句话生成全课程内容包（听说读写+游戏+图文音视频）",
  })
  ...
  async evaluate(
    @Body() body: { contentId: number; answers: any[]; age: number },
  ) {
    return this.aiService.evaluateLearning(
      body.contentId,
      body.answers,
      body.age,
    );
  ...
  async getSuggestion(@Query("ageRange") ageRange?: "3-4" | "5-6") {
    return this.aiService.generateSuggestion(
      null,
      ageRange === "3-4" ? 4 : ageRange === "5-6" ? 6 : 5,
    );
  }
}
