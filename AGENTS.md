...
- **Backend deploy:** Backend is managed by systemd user service `lingxi-backend.service` (config at `~/.config/systemd/user/lingxi-backend.service`). **CRITICAL**: Must use system node v24 (`/usr/bin/node dist/main`), NOT nvm node v22 — better-sqlite3 native addon must match the running binary's MODULE_VERSION (v24=137, v22=127). Build with: `npm run build`, then `systemctl --user restart lingxi-backend.service`. Port is 3001 (check `.env` — multiple agents may篡改 PORT causing 502). **CRITICAL**: better-sqlite3 native addon must match the running node binary's MODULE_VERSION — see lingxi-evolution skill pitfall #52. If the backend falls back to sql.js (check logs for "Falling back to sql.js"), registered users can't login and all API-created data is invisible. To rebuild better-sqlite3 for system node: `cd node_modules/better-sqlite3 && /usr/bin/node /usr/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js rebuild`.
