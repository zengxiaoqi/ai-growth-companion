...
export abstract class BaseAgent implements IAgent {
  ...
  abstract execute(input: string, context: AgentContext): Promise<ExecutionResult>;
  abstract executeStream(input: string, context: AgentContext): AsyncGenerator<StreamEvent>;
  ...
  protected normalizeToolArgs(
    ...
    if (meta.requiresChildId && !normalized.childId && context.childId != null) {
      normalized.childId = context.childId;
    }
    if (meta.requiresParentId && !normalized.parentId && context.parentId != null) {
      normalized.parentId = context.parentId;
    }
    if (meta.requiresAgeGroup && !normalized.ageGroup && context.ageGroup !== 'parent') {
  ...
  protected buildToolExecutionContext(context: AgentContext): ToolExecutionContext {
    return {
