...
export class AiService {
  ...
  async chat(params: ChatRequest): Promise<ChatResponse> {
    const { message, sessionId, context, viewerId, viewerType, targetChildId } =
      params;
    ...
    const ageGroup = this.normalizeAgentAgeGroup(
      this.agentExecutor.classifyAge(age),
    );
  ...
  private async executeRoutedChat(params: {
    ...
    if (this.frameworkOrchestrator) {
      const context = this.buildAgentContext({
        ...
        actorType:
          params.actorType ||
          (params.ageGroup === "parent" ? "parent" : "child"),
      });
      const result = await this.frameworkOrchestrator.route(
        params.message,
        context,
      );
      return {
  ...
  private async *executeRoutedChatStream(params: {
    ...
    if (this.frameworkOrchestrator) {
      ...
      for await (const event of this.frameworkOrchestrator.routeStream(
        params.message,
        context,
      )) {
        yield event as any;
  ...
  async canViewerAccessChild(params: {
    ...
    return this.usersService.canAccessChild(
      params.viewerId,
      params.viewerType,
      params.childId,
    );
  ...
  async getConversationSessionMessages(params: {
    ...
    const conversation = await this.conversationManager.getConversationByUuid(
      params.sessionId,
    );
    if (!conversation) {
      return {
  ...
  async exportCoursePack(params: {
    ...
    const title = this.toSafeFilename(
      String(pack.title || row.title || `course-pack-${row.id}`),
    );
    const capcutBody = JSON.stringify(
      this.buildCapCutPayload(pack, row.id),
      null,
      2,
    );
    ...
    const needNarrationMp3 =
      format === "narration_mp3" ||
      format === "bundle_zip" ||
      format === "teaching_video_mp4";
    const narrationMp3Body = needNarrationMp3
      ? await this.buildNarrationMp3(pack)
      : null;
    const needTeachingVideo =
      format === "teaching_video_mp4" ||
      (format === "bundle_zip" && !this.isUnitTestEnvironment());
    const teachingVideoMp4Body = needTeachingVideo
      ? await this.buildTeachingVideoMp4(pack, narrationMp3Body)
      : null;
  ...
  async renderTeachingVideoFromPack(
  ...
  async saveCoursePackVersion(params: {
    ...
    if (baseRow.sourceId == null) {
      await this.learningArchiveService.updateStudyPlanRecord(baseRow.id, {
    ...
    const parentId =
      params.viewerType === "parent" && params.viewerId != null
        ? params.viewerId
        : baseRow.parentId || undefined;
    const baseTitle = String(
      params.title ||
        nextPack?.title ||
        baseRow.title ||
        `Course Pack ${baseRow.id}`,
    );
  ...
  async enrichCoursePackBilingual(params: {
    ...
    const pack = this.deepCloneObject(
      (row.planContent || {}) as Record<string, any>,
    );
    ...
    const updatedPack = this.applyBilingualTranslations(
      pack,
      translations,
      overwrite,
    );
    ...
    if (childId != null) {
      try {
        ...
        const record = await this.learningArchiveService.createStudyPlanRecord({
          ...
          sourceType: "ai_course_pack",
          title: String(parsed.title || params.topic || "AI Course Pack").slice(
            0,
            180,
          ),
          planContent: parsed,
        ...
        await this.learningArchiveService.updateStudyPlanRecord(record.id, {
          sourceId: record.id,
        });
        ...
        this.logger.warn(
          `save course pack failed: ${error?.message || "unknown"}`,
        );
      }
    }
...
[
  ...
  private normalizeCoursePackExportFormats(
    formats?: CoursePackExportFormat[],
    ...
    const source: string[] =
      Array.isArray(formats) && formats.length > 0 ? formats : ["bundle_zip"];
    const normalized = source.filter((item): item is CoursePackExportFormat =>
      allowSet.has(item as CoursePackExportFormat),
    );
  ...
  private collectTextsForBilingual(
    pack: Record<string, any>,
    overwrite: boolean,
  ): string[] {
    const list: string[] = [];
    ...
    (Array.isArray(pack?.outcomes) ? pack.outcomes : []).forEach(
      (item: any, idx: number) =>
        pushText(
          item,
          Array.isArray(pack?.outcomesEn) ? pack.outcomesEn[idx] : undefined,
        ),
    );

    const shots = Array.isArray(pack?.videoLesson?.shots)
      ? pack.videoLesson.shots
      : [];
    ...
    const scenes = Array.isArray(pack?.visualStory?.scenes)
      ? pack.visualStory.scenes
      : [];
  ...
  private applyBilingualTranslations(
    ...
    if (pack?.summary)
      pack.summaryEn = translate(pack.summary, pack?.summaryEn);
    ...
    shots.forEach((shot: any) => {
      if (shot?.caption)
        shot.captionEn = translate(shot.caption, shot?.captionEn);
      if (shot?.narration)
        shot.narrationEn = translate(shot.narration, shot?.narrationEn);
    });
    ...
    scenes.forEach((scene: any) => {
      if (scene?.onScreenText)
        scene.onScreenTextEn = translate(
          scene.onScreenText,
          scene?.onScreenTextEn,
        );
      if (scene?.narration)
        scene.narrationEn = translate(scene.narration, scene?.narrationEn);
    });
    ...
    audioScript.forEach((item: any) => {
      if (item?.narration)
        item.narrationEn = translate(item.narration, item?.narrationEn);
    });

    return pack;
  }

  private async translateTextsToEnglish(
    texts: string[],
    ...
    const toTranslate = uniqueTexts.filter(
      (text) => !this.isMostlyEnglish(text),
    );
    if (toTranslate.length === 0) return results;

    const canUseLlm =
      this.llmClient.isConfigured &&
      typeof this.llmClient.generate === "function";
  ...
  private buildNarrationText(pack: Record<string, any>): string {
    ...
    if (lines.length === 0)
      lines.push("No narration content found in this course pack.");
    return lines.join("\n");
  }
  ...
  private collectNarrationScriptSegments(pack: Record<string, any>): string[] {
    ...
    const scenes = Array.isArray(pack?.visualStory?.scenes)
      ...
      : [];
    scenes.forEach((scene: any) =>
      pushText(scene?.narration || scene?.onScreenText),
    );
  ...
  private async buildNarrationMp3(
    pack: Record<string, any>,
  ): Promise<Buffer | null> {
    ...
    try {
      ...
      this.logger.warn(
        `build narration mp3 failed: ${error?.message || "unknown"}`,
      );
      return null;
    }
  }

  private async buildTeachingVideoMp4(
    pack: Record<string, any>,
    narrationMp3: Buffer | null,
  ): Promise<Buffer | null> {
    const ffmpegPath = this.getFfmpegExecutablePath();
    if (!ffmpegPath) return null;

    const tempRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "course-pack-video-"),
    );
    ...
    try {
      ...
      for (let index = 0; index < storyboard.length; index += 1) {
        ...
        const titleText = this.wrapSubtitleText(
          String(shot.headline || `镜头 ${index + 1}`).trim() ||
            `镜头 ${index + 1}`,
          16,
          1,
        );
        ...
        await fs.writeFile(
          path.join(tempRoot, subtitleFile),
          subtitleText,
          "utf8",
        );
        if (visualText) {
          await fs.writeFile(
            path.join(tempRoot, visualFile),
            visualText,
            "utf8",
          );
        }
        ...
        for (
          let tokenIndex = 0;
          tokenIndex < Math.min(5, themeStyle.motionTokens.length);
          tokenIndex += 1
        ) {
          const token = this.wrapSubtitleText(
            String(themeStyle.motionTokens[tokenIndex] || "").trim(),
            8,
            1,
          );
          if (!token) continue;
        ...
        if (safeDuration > 0.9) {
          const fadeOutStart = this.roundSeconds(
            Math.max(0.1, safeDuration - 0.28),
          );
          filterParts.push(`fade=t=out:st=${fadeOutStart}:d=0.28`);
        }
        const videoFilter = filterParts.join(",");
        const spriteAssets = await this.buildShotSpriteAssets(
          themeStyle,
          shot,
          index,
          tempRoot,
      ...
      const stitchedFile = await this.mergeSegmentsWithTransitions(
        ffmpegPath,
        tempRoot,
        segmentNames,
        segmentDurations,
      ...
      this.logger.warn(
        `build teaching video mp4 failed: ${error?.message || "unknown"}`,
      );
      return null;
    } finally {
  ...
  private resolvePackDurationSec(pack: Record<string, any>): number {
    ...
    const shotDuration = shots.reduce(
      (sum: number, shot: any) =>
        sum + this.toSafeNumber(shot?.durationSec, 8, 1, 180),
      0,
    );
    ...
    const sceneDuration = scenes.reduce(
      (sum: number, scene: any) =>
        sum + this.toSafeNumber(scene?.durationSec, 6, 1, 120),
      0,
    );
    if (sceneDuration > 0) return this.toSafeNumber(sceneDuration, 30, 3, 300);

    const declaredDuration = this.toSafeNumber(
      pack?.videoLesson?.durationSec,
      30,
      3,
      300,
    );
  ...
  private normalizeStoryboardHeadline(
    headline: any,
    subtitle: string,
    fallbackTitle: string,
  ): string {
    const rawHeadline = String(headline || "")
      .replace(/\s+/g, " ")
      .trim();
    const rawSubtitle = String(subtitle || "")
      .replace(/\s+/g, " ")
      .trim();
    const shouldPreferSubtitle =
      !rawHeadline ||
      /^(opening|concept|practice|wrap[\s-]?up|shot\s*\d+|scene\s*\d+|audio\s*\d+)$/i.test(
        rawHeadline,
      ) ||
  ...
  private compactStoryboardHeadline(
    text: string,
    fallbackTitle: string,
  ): string {
  ...
  private wrapSubtitleText(
    ...
    if (wrapped.length > limited.length) {
      ...
      if (tail.length >= 1) {
        limited[lastIdx] =
          `${tail.slice(0, Math.max(1, maxCharsPerLine - 3)).join("")}...`;
      } else {
        limited[lastIdx] = "...";
      }
    }
  ...
  private buildThemeAnimationProfile(
    ...
    if (
      /(山|河|湖|海|森林|地理|自然|海洋|river|ocean|mountain|lake|nature|earth)/.test(
        sourceText,
      )
    ) {
      palette = {
      ...
      /(数学|算数|数感|加法|减法|乘法|除法|几何|math|number|geometry|count)/.test(
        sourceText,
      )
    ) {
      palette = {
      ...
      /(汉字|识字|拼音|语文|阅读|写字|literacy|chinese|character|word|reading)/.test(
        sourceText,
      )
    ) {
      palette = {
      ...
      /(科学|实验|观察|探索|science|physics|chemistry|biology|planet)/.test(
        sourceText,
      )
    ) {
      palette = {
      ...
      /(英语|英文|单词|字母|english|alphabet|phonics)/.test(sourceText)
    ) {
      palette = {
        themeKind: "english",
  ...
  private extractThemeKeywords(text: string, max: number): string[] {
    const chunks =
      String(text || "").match(/[A-Za-z]{2,}|[0-9]+|[\u4e00-\u9fa5]{1,4}/g) ||
      [];
  ...
  private async renderSvgSpriteToPng(
    svg: string,
    outputPath: string,
  ): Promise<boolean> {
    try {
      ...
      this.logger.warn(
        `render sprite png failed: ${error?.message || "unknown"}`,
      );
      return false;
    }
  }
  ...
  private buildThemeSpriteSvg(
    ...
    const safeLabel = this.escapeXml(
      String(label || "学习").slice(0, variant === "main" ? 6 : 4),
    );
    ...
    const ring =
      variant === "main"
        ? `<ellipse cx="256" cy="428" rx="166" ry="34" fill="${style.shadow}" opacity="0.34" />`
        : "";
  ...
  private buildSpriteOverlayExpression(
    ...
    if (slot === "main") {
      if (themeStyle.themeKind === "nature")
        return `x=(main_w*0.04)+34*sin(t*0.42+${p}):y=(main_h*0.42)+18*cos(t*0.76+${p})`;
      if (themeStyle.themeKind === "math")
        return `x=(main_w*0.10)+26*sin(t*0.82+${p}):y=(main_h*0.38)+16*cos(t*0.94+${p})`;
      if (themeStyle.themeKind === "literacy")
        return `x=(main_w*0.08)+20*sin(t*0.56+${p}):y=(main_h*0.40)+12*cos(t*0.66+${p})`;
      if (themeStyle.themeKind === "science")
        return `x=(main_w*0.60)+26*sin(t*0.72+${p}):y=(main_h*0.26)+22*cos(t*0.84+${p})`;
      if (themeStyle.themeKind === "english")
        return `x=(main_w*0.11)+22*sin(t*0.62+${p}):y=(main_h*0.34)+14*cos(t*0.70+${p})`;
      return `x=(main_w*0.12)+24*sin(t*0.62+${p}):y=(main_h*0.38)+14*cos(t*0.72+${p})`;
    }

    if (themeStyle.themeKind === "nature")
      return `x=(main_w*0.70)+28*cos(t*0.54+${p}):y=(main_h*0.56)+12*sin(t*0.78+${p})`;
    if (themeStyle.themeKind === "math")
      return `x=(main_w*0.66)+18*sin(t*0.80+${p}):y=(main_h*0.58)+10*cos(t*0.90+${p})`;
    if (themeStyle.themeKind === "literacy")
      return `x=(main_w*0.68)+14*sin(t*0.48+${p}):y=(main_h*0.52)+10*cos(t*0.62+${p})`;
    if (themeStyle.themeKind === "science")
      return `x=(main_w*0.14)+30*sin(t*0.92+${p}):y=(main_h*0.54)+14*cos(t*0.88+${p})`;
    if (themeStyle.themeKind === "english")
      return `x=(main_w*0.68)+20*sin(t*0.74+${p}):y=(main_h*0.54)+10*cos(t*0.68+${p})`;
  ...
  private async mergeSegmentsWithTransitions(
    ...
    return this.concatSegmentsWithoutTransition(
      ffmpegPath,
      tempRoot,
      segmentNames,
    );
  ...
  private getFfmpegExecutablePath(): string | null {
    try {
      ...
      const installerPath =
        typeof installer?.path === "string" ? installer.path.trim() : "";
  ...
  private buildStoryboardCsv(pack: Record<string, any>): string {
    ...
    if (rows.length === 1)
      rows.push(["1", "note", "empty", "", "No storyboard data found", "", ""]);
    return rows
  ...
  private buildCapCutPayload(
    ...
    const timeline = shots.map((shot: any, index: number) => {
      ...
      return {
        ...
        transitionToNext: this.recommendTransition(
          index,
          shots.length,
          durationSec,
        ),
    ...
    const totalDurationSec = this.roundSeconds(
      timeline.reduce((sum, item) => sum + (item.durationSec || 0), 0),
    );
    ...
    const voiceOverSegments = timeline.map((item: any, index: number) => {
      const holdTailSec =
        item.pauseAfterSec > 0 ? Math.min(item.pauseAfterSec, 1) : 0;
      const endSec = this.roundSeconds(
        Math.max(item.startSec + 0.8, item.endSec - holdTailSec),
      );
      return {
        id: `voice-${index + 1}`,
        trackId: "voice-main",
    ...
    const transitionSfx = timeline
      ...
      .map((item: any, index: number) => {
        const cueStart = this.roundSeconds(
          Math.max(
            item.startSec + 0.2,
            item.endSec -
              this.toSafeNumber(item.transitionToNext?.durationSec, 0.5, 0, 3),
          ),
        );
        return {
    ...
    const pausePointsSec = timeline
      .filter((item: any) => item.pauseAfterSec > 0)
      .map((item: any) => this.roundSeconds(item.endSec));
    const beatPointsSec = timeline.map((item: any) =>
      this.roundSeconds(item.startSec),
    );
    if (safeTotalDurationSec > 0)
      beatPointsSec.push(this.roundSeconds(safeTotalDurationSec));
    ...
    return {
      ...
      capcutHint: {
        ...
        notes:
          "Use timeline start/end, transitions, and tracks as direct edit references.",
        recommendedWorkflow: [
  ...
  private collectSubtitleSegments(
    pack: Record<string, any>,
  ): Array<{ text: string; textEn: string; durationSec: number }> {
    const segments: Array<{
      text: string;
      textEn: string;
      durationSec: number;
    ...
    for (const shot of shots) {
      pushSegment(
        shot?.caption || shot?.narration,
        shot?.durationSec,
        shot?.captionEn || shot?.narrationEn,
      );
    }
    if (segments.length === 0) {
      const scenes = Array.isArray(pack?.visualStory?.scenes)
      ...
      for (const scene of scenes) {
        pushSegment(
          scene?.onScreenText || scene?.narration,
          scene?.durationSec,
          scene?.onScreenTextEn || scene?.narrationEn,
        );
      }
    }
    if (segments.length === 0) {
  ...
  private recommendTransition(
    ...
    totalShots: number,
    durationSec: number,
  ): { type: string; durationSec: number; note: string } {
    if (index >= totalShots - 1)
      return { type: "none", durationSec: 0, note: "last shot" };
    const sequence = ["crossfade", "slide_left", "zoom_in", "dip_to_white"];
    const type = sequence[index % sequence.length];
    const transitionDuration = this.roundSeconds(
      Math.min(1.2, Math.max(0.4, durationSec * 0.08)),
    );
    return {
      type,
      durationSec: transitionDuration,
      note: "gentle child-friendly transition",
    };
  }
...
请按以下JSON格式返回：
{
  ...
  "questions": ["问题1", "问题2", "问题3"]
        ...
        this.logger.warn(
          `LLM story generation failed, using template: ${error.message}`,
        );
      }
    }
