...
const SUPPORTED_TYPES: ActivityType[] = [
  "quiz",
  "true_false",
  "fill_blank",
  "matching",
...
const ANIMAL_TOPIC_RE =
  /(动物|农场|森林|海洋|宠物|家禽|家畜|小鸡|小鸭|小猫|小狗|animal|farm|zoo|pet|wild|livestock)/i;
const NUMBER_TOPIC_RE =
  /(数学|数字|认数|数数|加法|减法|math|number|count|arithmetic)/i;
const GENERIC_FALLBACK_RE =
  /(wake up|breakfast|go to school|go home|sun|moon|apple|fish|1\+1=3|太阳白天出现|起床|吃早饭|去上学|回家)/i;
...
export class GenerateActivityTool extends BaseTool<GenerateActivityArgs> {
  ...
  readonly metadata: ToolMetadata = {
    ...
    description:
      "生成互动学习活动，支持7种题型：quiz(选择题)、true_false(判断题)、fill_blank(填空题)、matching(配对)、connection(连线)、sequencing(排序)、puzzle(拼图)",
    inputSchema: {
      type: "object",
      properties: {
        type: {
  ...
  async execute(
    ...
    this.logger.log(
      `[generateActivity] start ${JSON.stringify({
        runId,
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      try {
        ...
        this.logger.log(
          `[generateActivity] llm_request ${JSON.stringify({ runId, attempt, topic: normalizedArgs.topic, type: normalizedArgs.type })}`,
        );
        ...
        if (!response || !response.trim()) {
          ...
          this.logger.warn(
            `[generateActivity] attempt_failed ${JSON.stringify({ runId, attempt, reason })}`,
          );
          continue;
        }
        ...
        if (!parsed) {
          ...
          this.logger.warn(
            `[generateActivity] attempt_failed ${JSON.stringify({ runId, attempt, reason, responsePreview: preview })}`,
          );
          continue;
        }

        const sanitized = this.sanitizeActivity(normalizedArgs, parsed);
        const alignment = this.validateTopicAlignment(
          normalizedArgs,
          sanitized,
        );
        if (!alignment.ok) {
          ...
          failures.push(`attempt ${attempt}: ${reason}`);
          reflections.push(
            `Your content did not match the topic "${normalizedArgs.topic}". Issue: ${reason}. Ensure every question/pair/item references the specific topic directly.`,
          );
          this.logger.warn(
            `[generateActivity] attempt_failed ${JSON.stringify({ runId, attempt, reason })}`,
          );
        ...
        this.logger.log(
          `[generateActivity] success ${JSON.stringify({ runId, attempt, topic: normalizedArgs.topic, type: normalizedArgs.type })}`,
        );
        ...
        this.logger.warn(
          `[generateActivity] attempt_exception ${JSON.stringify({ runId, attempt, reason })}`,
        );
      }
    }
    ...
    this.logger.error(
      `[generateActivity] failed_after_retries ${JSON.stringify({ runId, topic: normalizedArgs.topic, type: normalizedArgs.type, failures })}`,
    );
    return this.fail(
      `Unable to generate a topic-aligned activity. details=${detail}`,
    );
  }
    ...
    const reflectionNotes =
      reflections.length > 0
        ? [
            ...
            ...reflections.slice(-2).map((item) => `- ${item}`),
          ].join("\n")
        : "";
    ...
    switch (type) {
      case "quiz":
        return [
          ...
          "At least 3 options per question.",
          "correctIndex must be a zero-based number.",
        ];
      case "true_false":
        return [
          ...common,
      ...
      case "fill_blank":
        return [
          ...common,
          "Generate 3-5 fill-in sentences.",
          "Each sentence must contain a blank marker like ___.",
          "answer must appear in options.",
          "Do not reuse the same answer for all sentences.",
        ];
      case "matching":
        return [
          ...common,
          "Generate 4-8 pairs.",
          "left/right should be in the same knowledge domain.",
        ];
      case "connection":
        return [
          ...common,
          "Generate 4-8 connections.",
          "connections must reference existing ids.",
        ];
      case "sequencing":
        return [
          ...common,
          "Generate 4-6 ordered steps.",
          "order must be sequential and meaningful.",
        ];
      case "puzzle":
        return [
          ...common,
          "Generate 2x2 or 3x3 puzzle data.",
          "pieces should match grid size.",
        ];
      default:
        return common;
    }
  ...
  private topicRules(topic: string): string[] {
    ...
    if (this.isAnimalTopic(t)) {
      ...
      rules.push(
        "Do not reduce to pure counting; include real animal knowledge.",
      );
    }
    if (this.isNumberTopic(t)) {
      rules.push(
        "Include concrete numbers (for example 0-10 or Chinese numerals), not generic placeholders.",
      );
      rules.push(
        "For fill blanks, answers should be actual numbers or math words, not the full topic phrase.",
      );
    }
    if (t.includes("住") || t.includes("栖息"))
      rules.push("Must include habitat or where-animal-lives information.");
    if (t.includes("习性")) rules.push("Must include behavior or habits.");
    if (t.includes("成长") || t.includes("生命周期"))
      rules.push("Must include growth stages or life cycle.");
    if (t.includes("分类"))
      rules.push("Must include classification information.");
    if (t.includes("爱吃") || t.includes("食物"))
      rules.push("Must include diet information.");
    return rules;
  }
  ...
  private sanitizeQuiz(args: GenerateActivityArgs, raw: any) {
    const questions = (Array.isArray(raw?.questions) ? raw.questions : [])
      .map((q: any) => {
        ...
        const options = Array.isArray(q?.options)
          ? q.options.map((opt: any) => this.toText(opt)).filter(Boolean)
          : [];
    ...
    if (questions.length < 3)
      throw new Error(`quiz items too few: ${questions.length}`);
    return {
      type: "quiz",
  ...
  private sanitizeTrueFalse(args: GenerateActivityArgs, raw: any) {
    ...
    if (statements.length < 3)
      throw new Error(`true_false items too few: ${statements.length}`);
    return {
      type: "true_false",
  ...
  private sanitizeFillBlank(args: GenerateActivityArgs, raw: any) {
    const sentences = (Array.isArray(raw?.sentences) ? raw.sentences : [])
      .map((s: any) => {
        ...
        const options = Array.isArray(s?.options)
          ? s.options.map((opt: any) => this.toText(opt)).filter(Boolean)
          : [];
    ...
    if (sentences.length < 3)
      throw new Error(`fill_blank items too few: ${sentences.length}`);
    return {
      type: "fill_blank",
  ...
  private sanitizeMatching(args: GenerateActivityArgs, raw: any) {
    ...
    if (pairs.length < 3)
      throw new Error(`matching items too few: ${pairs.length}`);
    return {
      type: "matching",
  ...
  private sanitizeConnection(args: GenerateActivityArgs, raw: any) {
    ...
    if (
      ...
      throw new Error(
        `connection data too few: left=${leftItems.length}, right=${rightItems.length}, links=${connections.length}`,
      );
    }
    return {
      type: "connection",
  ...
  private sanitizeSequencing(args: GenerateActivityArgs, raw: any) {
    ...
    if (items.length < 3)
      throw new Error(`sequencing items too few: ${items.length}`);
    return {
      type: "sequencing",
  ...
  private sanitizePuzzle(args: GenerateActivityArgs, raw: any) {
    const rows = Math.max(
      2,
      Math.min(3, this.toSafeInt(raw?.gridSize?.rows, 2)),
    );
    const cols = Math.max(
      2,
      Math.min(3, this.toSafeInt(raw?.gridSize?.cols, 2)),
    );
    ...
    if (pieces.length < requiredPieces)
      throw new Error(
        `puzzle pieces too few: need=${requiredPieces}, got=${pieces.length}`,
      );
    return {
      type: "puzzle",
  ...
  private validateTopicAlignment(
    args: GenerateActivityArgs,
    activity: any,
  ): ValidationResult {
    ...
    if (this.isAnimalTopic(topic)) {
      const animalHits = Array.from(
        new Set(
          ANIMAL_TERMS.map((term) => term.toLowerCase()).filter((term) =>
      ...
      if (GENERIC_FALLBACK_RE.test(bodyContent))
        return { ok: false, reason: "generic fallback content detected" };
    }
    ...
    const memberHits = categoryMembers.filter((member) =>
      bodyContent.includes(member),
    );
  ...
  private getCategoryMembers(topic: string): string[] {
    const t = topic.toLowerCase();
    if (this.isAnimalTopic(t))
      return ANIMAL_TERMS.map((term) => term.toLowerCase());
    if (this.isNumberTopic(t))
      return NUMBER_TERMS.map((term) => term.toLowerCase());
    return [];
  }
  ...
  private validateContentQuality(
    ...
    const sentences = Array.isArray(activity?.sentences)
      ? activity.sentences
      : [];
    if (sentences.length < 3)
      return { ok: false, reason: "fill_blank sentences too few" };
    ...
    const answers = sentences
      .map((s: any) => this.toText(s?.answer).toLowerCase())
      .filter(Boolean);
    if (answers.length < 3)
      return { ok: false, reason: "fill_blank answers too few" };
    const uniqueAnswers = new Set(answers);
    if (uniqueAnswers.size < 2)
      return { ok: false, reason: "fill_blank answers have no diversity" };
  ...
  private requiredKeywordGroups(topic: string): string[][] {
    const groups: string[][] = [];
    if (topic.includes("住") || topic.includes("栖息"))
      groups.push(["住", "哪里", "栖息", "家"]);
    if (topic.includes("习性")) groups.push(["习性", "喜欢", "会", "通常"]);
    if (topic.includes("成长") || topic.includes("生命周期"))
      groups.push(["成长", "长大", "幼崽", "孵化", "阶段"]);
    if (topic.includes("分类")) groups.push(["分类", "属于", "哪一类"]);
    if (topic.includes("爱吃") || topic.includes("食物"))
      groups.push(["爱吃", "吃", "食物"]);
    if (topic.includes("农场"))
      groups.push(["农场", "奶牛", "小猪", "母鸡", "绵羊"]);
    if (topic.includes("森林"))
      groups.push(["森林", "狐狸", "鹿", "熊", "松鼠"]);
    return groups;
  }
  ...
  private extractTopicKeywords(topic: string): string[] {
    ...
    for (const seg of segments) {
      if (/^[\u4e00-\u9fa5]+$/.test(seg)) {
        const matchedCore = TOPIC_CORE_TERMS.filter(
          (term) =>
            /[\u4e00-\u9fa5]/.test(term) && seg.includes(term.toLowerCase()),
    ...
    return Array.from(
      new Set(collected.filter((t) => !TOPIC_STOP_WORDS.has(t))),
    ).slice(0, 8);
  }

  private resolveCorrectIndex(raw: any, options: string[]): number {
    const len = options.length;
    const textAnswer = this.toText(
      raw?.correctAnswer || raw?.answer || raw?.correctOption,
    );
    if (textAnswer) {
      const byText = options.findIndex(
        (opt) => opt === textAnswer || opt.includes(textAnswer),
      );
