...
@Injectable()
@RegisterTool()
export class GenerateVideoDataTool extends BaseTool<GenerateVideoDataArgs, TeachingVideoData> {
  ...
  async execute(args: GenerateVideoDataArgs, _context: ToolExecutionContext): Promise<ToolResult<TeachingVideoData>> {
  ...
  private sanitizeVideoData(raw: Record<string, any>, topic: string, expectedSlideCount: number): TeachingVideoData {
    const rawSlides = Array.isArray(raw?.slides) ? raw.slides : [];
    const slides = rawSlides.slice(0, expectedSlideCount).map((s: any, i: number) => this.sanitizeSlide(s, i));
    if (slides.length < 3) {
