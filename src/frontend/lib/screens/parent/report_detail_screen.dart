...
/// 报告详情页：展示孩子的学习报告详情
/// 包含：统计卡片、每日学习图表、技能进度、成就亮点、AI 学习建议
...
class _ReportDetailScreenState extends State<ReportDetailScreen> {
  ...
  // 领域配置
  static const _domains = <String, _DomainConfig>{
    'language': _DomainConfig(label: '语言', color: Color(0xFF006384)),
    'math': _DomainConfig(label: '数学', color: Color(0xFF586000)),
    'science': _DomainConfig(label: '科学', color: Color(0xFF705900)),
    'art': _DomainConfig(label: '艺术', color: Color(0xFFB9AE6E)),
    'social': _DomainConfig(label: '社交', color: Color(0xFFB02500)),
  };

  static const _periodTabs = [
    _PeriodTab(key: 'daily', label: '日报'),
    _PeriodTab(key: 'weekly', label: '周报'),
    _PeriodTab(key: 'monthly', label: '月报'),
  ];
  ...
  void _parseReport(Map<String, dynamic> report) {
    ...
    if (dailyRaw is List) {
      _dailyStats = dailyRaw
          ...
          .map((item) {
            ...
            return _DailyStat(
              label: date.isNotEmpty ? _formatDate(date) : '',
    ...
    if (insights is List) {
      _aiInsights = insights.whereType<String>().where((s) => s.isNotEmpty).toList();
    }
  }
        ...
        children: _periodTabs.map((tab) {
          ...
          return Expanded(
            child: GestureDetector(
              onTap: () => _onPeriodChanged(tab.key),
              child: Container(
  ...
  Widget _buildDailyChart() {
    final theme = Theme.of(context);
    final maxMinutes = _dailyStats.isEmpty
        ? 0
                ...
                child: BarChart(
                  BarChartData(
                    ...
                    barGroups: _dailyStats.asMap().entries.map((entry) {
                      ...
                      final isMax = stat.minutes == maxMinutes;
            ...
            if (_achievements.isEmpty)
              ...
              GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final percent =
                      (ach.progress / ach.totalRequired * 100).clamp(0.0, 100.0);
                  return Container(
                    decoration: BoxDecoration(
                      color: ach.unlocked
          ...
          children: [
            ...
            if (_aiInsights.isEmpty)
              ...
              if (_aiInsights.length > 1) ...[
                const SizedBox(height: 12),
                ..._aiInsights.skip(1).take(3).map((insight) {
                  return Padding(
...
class _DomainConfig {
  ...
  const _DomainConfig({required this.label, required this.color});
}
...
class _PeriodTab {
  ...
  const _PeriodTab({required this.key, required this.label});
}
