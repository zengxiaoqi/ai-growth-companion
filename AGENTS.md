...
- **Modules** in `src/modules/`: auth (JWT), users, contents, learning, abilities, achievements, ai, parent, recommend, report, game, voice, emergency, assignment, notification, sse, reward (积分奖惩), poetry (古诗词), video-download (视频下载), public-api (公共API代理+水果数据), bento (成长报告/智能文档)
...
- **Flutter Web build:** Output at `src/frontend/build/web/`. nginx serves this directory directly — no rsync to `backend/public/` needed. Just run `cd src/frontend && flutter build web && bash post-build-web.sh` and the files are live. The post-build script version-caches `main.dart.js` → `main.dart.vN.js`, pre-compresses `.gz` files, adds `.catch()` error handling, and updates `flutter_bootstrap.js` references.
...
- **Flutter Web deploy:** `cd src/frontend && flutter build web && bash post-build-web.sh` — files served directly from `build/web/` by nginx. No rsync needed. The post-build script (1) version-caches `main.dart.js` → `main.dart.vN.js`, (2) adds `.catch()` error handling to `load()` call, (3) pre-compresses `.gz` files for gzip_static, (4) updates `flutter_bootstrap.js` references and cleans old versioned files (keeps latest 2). Note: Keeps canvaskit renderer (Flutter 3.41+ builds for canvaskit by default, not html).
- **React Web deploy:** `cd src/frontend-web && npm run build` — output at `dist/` served directly by nginx on :8081.
- **Backend deploy:** Backend is managed by systemd user service `lingxi-backend.service` (config at `~/.config/systemd/user/lingxi-backend.service`). The service uses nvm node v22 (`/home/zxq/.nvm/versions/node/v22.22.0/bin/node`). Build with: `npm run build`, then `systemctl --user restart lingxi-backend.service`. Port is 3001 (check `.env`). **CRITICAL**: better-sqlite3 native addon must match the running node binary's MODULE_VERSION — see lingxi-evolution skill pitfall #52. If the backend falls back to sql.js (check logs for "Falling back to sql.js"), registered users can't login and all API-created data is invisible. **sql.js fallback fix (2026-07-23)**: The `resolveSqliteDriver()` function now uses `autoSave: true` + explicit `database: lingxi.db` path, so even sql.js fallback persists data to disk. To rebuild better-sqlite3: `cd node_modules/better-sqlite3 && /usr/bin/node /usr/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js rebuild`.
