...
export class ReportService {
  ...
  private async getDailyStats(userId: number, dateRange: { start: Date; end: Date }, period: string) {
    const records = await this.learningRecordRepository.find({
      where: {
        userId,
        startedAt: Between(dateRange.start, dateRange.end),
    ...
    for (const record of records) {
      ...
      const existing = dayMap.get(dayKey) || { totalTime: 0, completed: 0, scores: [] as number[] };
    ...
    const result: { date: string; totalTime: number; completedLessons: number; averageScore: number }[] = [];
  ...
  private async getSkillProgress(userId: number): Promise<Record<string, number>> {
    ...
    const domainProgress: Record<string, number> = {
      language: 0, math: 0, science: 0, art: 0, social: 0,
    };
  ...
  async getAbilityTrend(userId: number, weeks: number) {
    ...
    const result: { week: string; language: number; math: number; science: number; art: number; social: number }[] = [];
    ...
    for (const weekNum of sortedWeeks) {
      ...
      const avg = (domain: string) => {
        ...
        return scores.length > 0 ? Math.round((scores.reduce((s, v) => s + v, 0) / scores.length) * 100) / 100 : 0;
      };
      result.push({
        week: `第${weekNum}周`,
