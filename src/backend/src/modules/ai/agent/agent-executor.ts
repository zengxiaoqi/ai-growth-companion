...
function isSupportedActivityType(
  value: unknown,
): value is SupportedActivityType {
...
export class AgentExecutor {
  ...
  buildSystemPrompt(
    ...
    const contextHints = [
      "## Runtime Context",
      executionContext?.childId != null
        ? `- Current childId: ${executionContext.childId}`
        : "",
      executionContext?.parentId != null
        ? `- Current parentId: ${executionContext.parentId}`
        : "",
    ...
    if (ageGroup === "parent")
      return `${systemPromptParent(childName)}\n\n${contextHints}`;
    if (ageGroup === "3-4")
      return `${systemPrompt34(childName)}\n\n${contextHints}`;
    if (ageGroup === "5-6")
  ...
  async execute(
    ...
    const rawHistory =
      await this.conversationManager.buildMessageArray(sessionId);
    ...
    for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
      ...
      if (response.toolCalls && response.toolCalls.length > 0) {
        ...
        await this.conversationManager.addMessage(
          sessionId,
          "assistant",
          stripThinking(response.content || ""),
          {
            toolCalls: response.toolCalls,
          },
        );
  ...
  async *executeStream(
    ...
    yield {
      type: "token",
      content: "我思考了很久，暂时想不出好的回答。换个问题试试吧~ 🌟",
    };
  ...
  private inferActivityType(
    ...
    if (
      ...
      (Array.isArray(value.leftItems) && Array.isArray(value.rightItems))
    )
      return "connection";
