...
describe('GenerateCoursePackTool scene output', () => {
  ...
  it('adds native watch/write/practice scene documents even when llm omits them', async () => {
    ...
    expect(result.practice?.scene?.scenes?.[1]?.fallbackActivity?.activityType).toBe(
      result.game?.activityType,
    );
    expect(result.practice?.scene?.scenes?.[1]?.fallbackActivity?.activityData).toEqual(
      result.game?.activityData,
    );
  });
    ...
    llmClient.generate.mockResolvedValueOnce(
      JSON.stringify({
        ...
        modules: {
          listening: {
            goal: 'listen',
            audioScript: [{ segment: 'intro', narration: '河字', durationSec: 8 }],
            questions: [],
          },
          speaking: {
            goal: 'speak',
