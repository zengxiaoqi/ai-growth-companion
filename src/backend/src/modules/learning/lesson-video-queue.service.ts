...
export class LessonVideoQueueService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly cacheTtlHours = this.toInt(
    process.env.VIDEO_CACHE_TTL_HOURS,
    72,
    1,
    24 * 30,
  );
  private readonly maxRetries = this.toInt(
    process.env.VIDEO_TASK_MAX_RETRIES,
    2,
    0,
    5,
  );
  ...
  private readonly downloadMaxBytes =
    this.toInt(process.env.VIDEO_PROVIDER_DOWNLOAD_MAX_MB, 80, 5, 1024) *
    1024 *
    1024;
  ...
  private readonly createTaskIdPaths = this.resolvePathList(
    "VIDEO_PROVIDER_CREATE_TASK_ID_PATHS",
    ["taskId", "jobId", "id", "data.taskId", "data.id"],
  );
  private readonly statusStatePaths = this.resolvePathList(
    "VIDEO_PROVIDER_STATUS_STATE_PATHS",
    ["status", "state", "data.status", "result.status"],
  );
  ...
  private readonly statusErrorPaths = this.resolvePathList(
    "VIDEO_PROVIDER_STATUS_ERROR_PATHS",
    ["error", "message", "data.error", "result.error"],
  );
  ...
  private async resetOrphanedProcessingTasks(): Promise<void> {
    try {
      ...
      if (result.affected && result.affected > 0) {
        this.logger.warn(
          `Reset ${result.affected} orphaned 'processing' task(s) to 'failed'`,
        );
      }
    } catch (error: any) {
  ...
  private async processNextTask(): Promise<void> {
    ...
    try {
      ...
      const expiresAt = new Date(
        now.getTime() + this.cacheTtlHours * 3600 * 1000,
      );
  ...
  private async generateByThirdParty(
    ...
    const directVideoUrl = this.pickFirstString(
      createJson,
      this.createVideoUrlPaths,
    );
    if (directVideoUrl) {
    ...
    const providerTaskId = this.pickFirstString(
      createJson,
      this.createTaskIdPaths,
    );
    if (!providerTaskId) {
    ...
    const statusPathTemplate =
      process.env.VIDEO_PROVIDER_STATUS_PATH || "/v1/video/tasks/{taskId}";
    for (let i = 0; i < this.providerPollMaxTimes; i++) {
      await this.sleep(this.providerPollIntervalMs);
      const statusPath = statusPathTemplate.replace(
        "{taskId}",
      ...
      const providerProgress = this.pickFirstNumber(
        statusJson,
        this.statusProgressPaths,
      );
      ...
      if (providerStatus === "failed") {
        const errMsg =
          this.pickFirstString(statusJson, this.statusErrorPaths) ||
  ...
  private async downloadRemoteVideo(url: string): Promise<Buffer> {
    ...
    if (
      ...
      contentLength > this.downloadMaxBytes
    ) {
  ...
  private async writeVideoToCache(
    cacheKey: string,
    buffer: Buffer,
  ): Promise<string> {
    await fs.mkdir(this.storageDir, { recursive: true });
  ...
  private async findReusableTask(
    ...
    if (
      task.status === "failed" &&
      (task.attemptCount || 0) <= this.maxRetries
    ) {
      task.status = "pending";
      task.errorMessage = null;
      task.progress = 0;
  ...
  private buildPackPayloadFromContent(content: Content): Record<string, any> {
    const raw =
      typeof content.content === "string"
        ? this.tryParseJson(content.content)
        : content.content;
    const lesson = raw && typeof raw === "object" ? raw : {};
    const steps = Array.isArray((lesson as any).steps)
      ? (lesson as any).steps
      : [];
  ...
  private buildPayloadFromLegacyContent(
    ...
    return {
      ...
      visualStory:
        allScenes.length > 0
          ? { scenes: allScenes, style: "colorful-educational" }
          : {},
      ...
      modules: {
        ...
        reading:
          keywords.length > 0
            ? { keywords, goal: `学习${topic}的关键词语` }
            : {},
        writing: {},
        game: gamePairs.length > 0 ? { matchingPairs: gamePairs } : {},
        quiz:
          questions.length > 0 ? { questions, title: `${topic}小测验` } : {},
      },
    };
  }
        ...
        return segments.slice(0, 8).map((segment: string, idx: number) => {
          ...
          return {
            ...
            animationParams: {
              ...
              characters:
                characters.length > 0 ? characters : ["老师", "小朋友"],
  ...
  private extractKeyPhrase(text: string, topic: string): string {
    ...
    const meaningful = phrases.find(
      (p) => !["小朋友", "我们", "一起", "看看", "今天"].includes(p),
    );
  ...
  private buildLegacyVideoLesson(
    ...
    const shots = storyItems.slice(0, 6).map((item: any, idx: number) => {
      ...
      const firstSentence =
        text.split(/[。！？]/)[0]?.trim() || text.slice(0, 60);
      return {
    ...
    return shots.length > 0
      ? {
          ...
          durationSec: shots.reduce(
            (s: number, sh: any) => s + (sh.durationSec || 10),
            0,
          ),
          shots,
        }
  ...
  private extractGamePairsFromLegacy(
    contentItems: any[],
    ...
    for (const block of gameBlocks) {
      ...
      for (const cat of categories.slice(0, 4)) {
        if (typeof cat === "string" && cat.includes(":")) {
          ...
          if (left && right)
            pairs.push({ left: left.slice(0, 12), right: right.slice(0, 12) });
        }
      }
    }
  ...
  private buildProviderCreateBody(
    ...
    const mode = String(
      process.env.VIDEO_PROVIDER_CREATE_BODY_MODE || "generic",
    )
      .trim()
      .toLowerCase();
    ...
    if (mode === "wrapped_payload") {
      const inputKey =
        String(process.env.VIDEO_PROVIDER_CREATE_INPUT_KEY || "input").trim() ||
        "input";
      return {
        [inputKey]: payload,
        metadata: {
  ...
  private normalizeProviderStatus(statusRaw?: string): ProviderStatus {
    ...
    if (
      ["running", "processing", "in_progress", "started", "progress"].includes(
        status,
      )
    )
      return "processing";
    if (
      ["completed", "done", "success", "succeeded", "finished"].includes(status)
    )
      return "completed";
    if (
      ["failed", "error", "canceled", "cancelled", "timeout"].includes(status)
    )
      return "failed";
    return "unknown";
  }
