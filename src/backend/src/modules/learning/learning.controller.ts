...
export class LearningController {
  ...
  private resolveChildId(req: any, childId?: string): number {
    const viewerType = req.user?.type;
    const numeric =
      viewerType === "child" ? Number(req.user?.sub) : Number(childId);
    ...
    if (updatedRecord) {
      try {
        ...
        await this.learningTracker.recordActivity({
          ...
          score:
            typeof updatedRecord.score === "number" ? updatedRecord.score : 0,
  ...
  async modifyLesson(
    ...
    return this.lessonContentService.modifyDraft(
      +id,
      parentId,
      body.modification,
      {
  ...
  async confirmLesson(
    ...
    await this.assertAccessToChild(req, body.childId);
    return this.lessonContentService.confirmAndPublish(
      +id,
      parentId,
      body.childId,
    );
  ...
  async getLessonTeachingVideoTask(
    ...
    const task = await this.lessonVideoQueue.getTask(
      +id,
      numericTaskId,
      resolvedChildId,
    );
  ...
  async getLessonTeachingVideo(
    ...
    const task =
      Number.isInteger(numericTaskId) && numericTaskId > 0
        ? await this.lessonVideoQueue.getTask(
            +id,
            numericTaskId,
            resolvedChildId,
          )
  ...
  async approveVideo(
    ...
    task.rejectionReason = body.approved
      ? null
      : (body.feedback || "").slice(0, 500) || null;
