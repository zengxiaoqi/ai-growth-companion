...
- **Flutter Web deploy:** `cd src/frontend && flutter build web && bash post-build-web.sh` — files served directly from `build/web/` by nginx. No rsync needed. The post-build script (1) adds `useLocalCanvasKit: true` + switches renderer to `html` (eliminates 7MB WASM canvaskit download), (2) version-caches `main.dart.js` → `main.dart.vN.js`, (3) adds `.catch()` error handling to `load()` call, (4) pre-compresses `.gz` files for gzip_static, (5) updates `flutter_bootstrap.js` references and cleans old versioned files (keeps latest 2).
