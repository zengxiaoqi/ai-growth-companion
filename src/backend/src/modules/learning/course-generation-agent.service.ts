...
export class CourseGenerationAgentService {
  ...
  async generateCoursePack(args: CourseAgentArgs): Promise<Record<string, any> | null> {
    if (!this.toolRegistry.has('generateCoursePack')) {
      this.logger.warn('[course-designer] generateCoursePack tool is not registered');
      return null;
    }
    ...
    if (!coursePack) {
      this.logger.warn(`[course-designer] no course pack returned for topic="${args.topic}"`);
      return null;
    }
  ...
  private buildSystemPrompt(context: AgentContext): string {
    ...
    const skills = this.skillRegistry.getSkillsForAgent(courseDesignerDefinition.allowedSkills);
    if (skills.length > 0) {
      prompt = `${prompt}\n\n## Skills\n\n${skills
