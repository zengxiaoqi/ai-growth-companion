...
describe("EnqueueTeachingVideoTool", () => {
  ...
  beforeEach(() => {
    ...
    tool = new EnqueueTeachingVideoTool(
      queueService as any,
      usersService as any,
    );
  });
  ...
  it("rejects invalid engine", async () => {
    const result = await tool.execute(
      { lessonId: 12, childId: 2, engine: "invalid" as any },
      {
        ageGroup: "parent",
        conversationId: "conv-1",
      } as any,
  ...
  it("rejects unauthorized parent-child access", async () => {
    ...
    const result = await tool.execute(
      { lessonId: 12, childId: 2, engine: "auto", parentId: 99 },
      {
        ageGroup: "parent",
        parentId: 99,
        conversationId: "conv-1",
  ...
  it("creates async task and returns task payload", async () => {
    ...
    const result = await tool.execute(
      { lessonId: 12, engine: "auto", force: true },
      {
        childId: 2,
        ageGroup: "parent",
        conversationId: "conv-1",
