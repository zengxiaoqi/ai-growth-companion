...
export default function MatchingGame({ data, onComplete }: MatchingGameProps) {
  ...
  const cards = useMemo(() => {
    const leftCards = pairs.map((p: any) => ({
      id: `L-${p.id}`,
      pairId: p.id,
      side: 'left' as const,
      label: p.left,
    ...
    const rightCards = pairs.map((p: any) => ({
      id: `R-${p.id}`,
      pairId: p.id,
      side: 'right' as const,
      label: p.right,
      ...
      if (first.pairId === second.pairId && first.side !== second.side) {
        ...
        if (newMatched.size === cards.length) {
          ...
          onComplete({
            ...
            interactionData: { attempts: attempts + 1 },
          });
        }
      } else {
    ...
    <div className="space-y-4">
      <div className="text-center">
        ...
        <p className="text-sm text-on-surface-variant">
          翻开卡片找到配对！{matched.size / 2} / {pairs.length}
        </p>
      </div>
        ...
        {cards.map((card) => {
          ...
          return (
            <motion.button
              ...
              whileHover={
                reducedMotion || (isRevealed && !isMatched) ? undefined : { scale: 1.05, y: -2 }
              }
              ...
              className={cn(
                ...
                isMatched &&
                  'bg-primary-container border-primary text-on-primary-container opacity-60',
                isRevealed &&
                  !isMatched &&
                  'bg-surface-container-lowest border-primary text-on-surface ring-2 ring-primary ring-offset-2',
                !isRevealed &&
                  !isMatched &&
                  'bg-surface-container border-outline-variant/30 text-on-surface-variant hover:border-primary/50 cursor-pointer shadow-sm',
