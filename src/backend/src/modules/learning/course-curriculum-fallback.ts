...
function scoreRecord(record: CurriculumRecord, topic: string, domain?: CourseDomain): number {
...
function buildSeed(record: CurriculumRecord, topic: string): CourseCurriculumSeed {
  const objectives = toTextArray(record.objectives).slice(0, 4);
  const blocks = Array.isArray(record.content) ? record.content.filter(Boolean) as Record<string, any>[] : [];
  ...
  const practiceTasks = unique([
    ...
    ...matchingPairs.slice(0, 2).map((pair) => `说一说${pair.left}${pair.right}`),
  ]).slice(0, 4);

  return {
    summary: objectives.join('；') || `围绕${topic}进行更具体的启蒙学习。`,
    outcomes: objectives.length > 0 ? objectives : teachingUnits.slice(0, 3).map((unit) => `认识${unit}的特点`),
    teachingUnits: teachingUnits.slice(0, 4),
    unitFacts,
    readingText: lessonTexts.join('\n\n').slice(0, 320) || `今天我们一起学习${topic}。`,
    readingKeywords: readingKeywords.length > 0 ? readingKeywords : [topic],
    listeningQuestions: unique(quizItems.map((item) => item.question)).slice(0, 3),
...
function extractExampleFacts(blocks: Record<string, any>[]): Record<string, string> {
  ...
  return Object.fromEntries(pairs.filter((entry) => entry.unit && entry.fact).map((entry) => [entry.unit, entry.fact]));
}
  ...
  return blocks.flatMap((block) => {
    ...
    return questions
      .map((item: any) => {
        ...
        const options = Array.isArray(item?.options) ? item.options.map((value: any) => toText(value)).filter(Boolean) : [];
...
function buildMatchingPairs(
  ...
  const fromQuiz = quizItems
    .map((item, index) => ({
      left: normalizeUnit(item.question.replace(/[？?].*$/, '')) || `知识点${index + 1}`,
      right: simplifyFact(item.answer),
    }))
    .filter((item) => item.left && item.right)
    .slice(0, 4);

  return fromFacts.length > 0 ? uniquePairs([...fromFacts, ...fromQuiz]).slice(0, 4) : fromQuiz;
}

function extractTopicTerms(source: string): string[] {
  const rawTerms = toText(source)
    .match(/[\u4e00-\u9fff]{1,6}/g) || [];
...
function uniquePairs(values: Array<{ left: string; right: string }>): Array<{ left: string; right: string }> {
