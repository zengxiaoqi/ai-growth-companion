...
export class ReportController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('trend')
  async getAbilityTrend(@Query('userId') userId: string, @Query('weeks') weeks: string = '6') {
    return this.reportService.getAbilityTrend(+userId, +weeks);
  }

  @UseGuards(JwtAuthGuard)
  @Get('recent-skills')
  async getRecentSkills(@Query('userId') userId: string, @Query('limit') limit: string = '3') {
