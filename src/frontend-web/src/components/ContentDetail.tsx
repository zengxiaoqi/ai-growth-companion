...
export default function ContentDetail({
  ...
  useEffect(() => {
    ...
    const startedAt =
      learningRecord.startedAt || learningRecord.startTime || new Date().toISOString();
        ...
        <main className="mx-auto w-full max-w-4xl space-y-5 px-4 py-6 md:px-6">
          {content.thumbnail ? (
            <Card className="relative overflow-hidden p-0">
              ...
              <div
                className={cn(
                  'absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black',
                  domainMeta.badgeClass,
                )}
              >
          ...
          <Card className="space-y-4 p-5">
            <div>
              ...
              {content.subtitle ? (
                <p className="mt-1 text-sm text-on-surface-variant">{content.subtitle}</p>
              ) : null}
            </div>
            ...
            <Card className="space-y-3 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {content.mediaUrls.map((url, index) => (
                  <div
                    key={index}
                    className="overflow-hidden rounded-xl border border-outline-variant/20 bg-surface"
                  >
                    <img
                      src={url}
                      alt={`学习资源 ${index + 1}`}
              ...
              <div className="relative z-10">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <div className="inline-flex items-center gap-1 rounded-full bg-primary-container/25 px-3 py-1.5 text-sm font-bold">
                    <CheckCircle className="h-4 w-4" />+{score} 积分
                  </div>
                  <div className="inline-flex items-center gap-1 rounded-full bg-primary-container/25 px-3 py-1.5 text-sm font-bold">
          ...
          <div className="mx-auto w-full max-w-4xl">
            {!learningRecord ? (
              <div className="space-y-2">
                {(startError || !childId) && (
                  <p
                    className="flex items-center justify-center gap-1.5 text-sm font-semibold text-error"
                    role="alert"
                  >
