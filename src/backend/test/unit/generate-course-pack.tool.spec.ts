...
describe('GenerateCoursePackTool', () => {
  ...
  beforeEach(() => {
    ...
    mockedGetCoursePackCurriculumSeed = getCoursePackCurriculumSeed as jest.MockedFunction<
  ...
  it('expands explicit literacy units into per-item teaching shots', async () => {
    ...
    const audioSegments =
      result.modules?.listening?.audioScript?.map((item: any) => String(item.segment)) || [];

    expect(shotNames).toEqual(expect.arrayContaining(['天字讲解', '地字讲解', '人字讲解']));
    expect(
      narrations.some((text: string) => text.includes('“天”字') || text.includes('”天”字')),
    ).toBe(true);
    expect(
      narrations.some((text: string) => text.includes('“地”字') || text.includes('”地”字')),
    ).toBe(true);
    expect(
      narrations.some((text: string) => text.includes('“人”字') || text.includes('”人”字')),
    ).toBe(true);
    expect(audioSegments).toEqual(expect.arrayContaining(['天字讲解', '地字讲解', '人字讲解']));
  });
  ...
  it('uses curriculum seed for fallback summary and review narration', async () => {
    ...
    const audioNarrations =
      result.modules?.listening?.audioScript?.map((a: any) => String(a.narration)) || [];
