...
export class GenerateActivityTool {
  ...
  async execute(args: GenerateActivityArgs): Promise<string> {
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      try {
        ...
        this.logger.log(
          `[generateActivity] llm_request ${JSON.stringify({
            runId,
            attempt,
            topic: normalizedArgs.topic,
            type: normalizedArgs.type,
        ...
        this.logger.log(
          `[generateActivity] llm_response ${JSON.stringify({
            runId,
            attempt,
            topic: normalizedArgs.topic,
            type: normalizedArgs.type,
        ...
        if (!parsed) {
          ...
          this.logger.warn(
            `[generateActivity] attempt_failed ${JSON.stringify({
              runId,
              attempt,
              reason,
              topic: normalizedArgs.topic,
        ...
        this.logger.log(
          `[generateActivity] success ${JSON.stringify({
            runId,
            attempt,
            topic: normalizedArgs.topic,
            type: normalizedArgs.type,
        ...
        failures.push(`attempt ${attempt}: ${reason}`);
        this.logger.warn(
          `[generateActivity] attempt_exception ${JSON.stringify({
            runId,
            attempt,
            reason,
            topic: normalizedArgs.topic,
    ...
    this.logger.error(
      `[generateActivity] failed_after_retries ${JSON.stringify({
        runId,
        topic: normalizedArgs.topic,
        type: normalizedArgs.type,
    ...
    return {
      ...
      counts: {
        questions: Array.isArray(activity?.questions)
          ...
          : 0,
        statements: Array.isArray(activity?.statements)
          ? activity.statements.length
          : 0,
        sentences: Array.isArray(activity?.sentences)
          ? activity.sentences.length
          : 0,
        pairs: Array.isArray(activity?.pairs) ? activity.pairs.length : 0,
        leftItems: Array.isArray(activity?.leftItems)
          ? activity.leftItems.length
          : 0,
        rightItems: Array.isArray(activity?.rightItems)
          ? activity.rightItems.length
          : 0,
        connections: Array.isArray(activity?.connections)
  ...
  private topicRules(topic: string): string[] {
    ...
    if (t.includes("\u4e60\u6027"))
      rules.push("Must include behavior or habits.");
  ...
  private sanitizeQuiz(args: GenerateActivityArgs, raw: any) {
    ...
    if (normalized.length < 3)
      throw new Error(`quiz items too few: ${normalized.length}`);
  ...
  private validateTopicAlignment(
    ...
    if (!bodyContent)
      return { ok: false, reason: "activity body is empty", debug: debugBase };
  ...
  private validateContentQuality(
    ...
    if (
      sentences.some((s: any) => !this.hasBlankMarker(this.toText(s?.text)))
    ) {
    ...
    if (this.isNumberTopic(topicText)) {
      const bodyContent = this.collectActivityBodyText(activity).toLowerCase();
      const termHits = NUMBER_TERMS.filter((term) =>
        bodyContent.includes(term.toLowerCase()),
      ).length;
      const digitHits = (bodyContent.match(/\b\d+\b/g) || []).length;
      if (termHits + digitHits < 3) {
        return {
          ok: false,
          reason: "number topic lacks concrete number content",
        };
      }
    }
  ...
  private requiredKeywordGroups(topic: string): string[][] {
    ...
    if (topic.includes("\u4e60\u6027"))
      groups.push(["\u4e60\u6027", "\u559c\u6b22", "\u4f1a", "\u901a\u5e38"]);
    if (
      topic.includes("\u6210\u957f") ||
      topic.includes("\u751f\u547d\u5468\u671f")
    )
    ...
    if (topic.includes("\u5206\u7c7b"))
      groups.push(["\u5206\u7c7b", "\u5c5e\u4e8e", "\u54ea\u4e00\u7c7b"]);
  ...
  private extractJsonObjectWithMeta(text: string): JsonExtractionResult {
    ...
    try {
      ...
      parseErrors.push(
        `direct_parse: ${this.toText(error?.message, "failed")}`,
      );
    }
    ...
    if (codeBlockMatch?.[1]) {
      ...
      try {
        ...
        parseErrors.push(
          `code_block_parse: ${this.toText(error?.message, "failed")}`,
        );
      }
    }
    ...
    if (firstBrace >= 0 && lastBrace > firstBrace) {
      ...
      try {
        ...
        parseErrors.push(
          `brace_slice_parse: ${this.toText(error?.message, "failed")}`,
        );
      }
    }
  ...
  private toBoolean(value: any): boolean {
    ...
    if (
      ["true", "1", "yes", "y", "\u662f", "\u5bf9", "\u6b63\u786e"].includes(t)
    )
      return true;
    if (
      ["false", "0", "no", "n", "\u5426", "\u9519", "\u9519\u8bef"].includes(t)
    )
      return false;
    return false;
  }
