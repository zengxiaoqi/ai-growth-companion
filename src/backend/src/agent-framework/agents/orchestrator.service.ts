...
export class OrchestratorService implements OnApplicationBootstrap {
  ...
  private buildRoutePlan(input: string, context: AgentContext, selectedType: AgentType): RoutePlan {
  ...
  private mergeCoordinationResult(
    ...
    const text = [
      ...
      '',
      ...sections.map((section) => [`## ${section.title}`, section.result.response, ''].join('\n')),
    ].join('\n');
  ...
  private buildPrimaryIntegrationPrompt(
    ...
    const sectionText =
      sections.length > 0
        ? sections
            .map((section) => [`### ${section.title}`, section.result.response].join('\n'))
            .join('\n\n')
  ...
  private getFilteredToolDefinitions(definition: AgentDefinition): LlmToolDefinition[] | undefined {
  ...
  private injectSkills(systemPrompt: string, allowedSkills?: string[]): string {
    ...
    this.logger.debug(`Injecting skill index: ${skills.map((s) => s.definition.id).join(', ')}`);
